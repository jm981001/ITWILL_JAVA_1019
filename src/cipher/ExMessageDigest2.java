...
public class ExMessageDigest2 {

	public static void main(String[] args) {
		// MyMessageDigest 객체 생성
		// => 단방향 해싱 함수 : "MD5", "SHA-1", "SHA-256", "SHA-512" 등 실제 함수명을 정확히 작성
		String hashAlgorithm = "SHA-256";
		MyMessageDigest2 md = new MyMessageDigest2(hashAlgorithm);
		
		// 생성된 암호문 리턴받아 저장
		String securePasswd = md.hashing("1234");
//		System.out.println(securePasswd.length());
		
		// 새로운 패스워드 입력받아 저장(평문)
		String passwd = "1234";
		
		// if 문을 사용하여 MyMessageDigest2 객체의 matches() 메서드를 호출하여 
		// 이전에 생성된 암호문과 평문 암호 비교
		if(md.matches(securePasswd, passwd)) {
...
class MyMessageDigest2 {
	...
	public MyMessageDigest2(String hashAlgorithm) {
